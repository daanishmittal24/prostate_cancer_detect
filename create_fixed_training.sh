...
echo "=== Creating Python 3.6 Compatible Training Script ==="

# Create a Python 3.6 compatible version
cat > train_robust.py << 'EOF'
import os
import time
import torch
...
# Import robust data loader
try:
    from data_loader_robust import get_dataloaders_robust
    USE_ROBUST_LOADER = True
    print("Using robust data loader (handles corrupted images)")
except ImportError:
    from data_loader import get_dataloaders
    USE_ROBUST_LOADER = False
    print("Warning: Using standard data loader (may fail on corrupted images)")
...
def parse_args():
    ...
    parser.add_argument('--train-csv', type=str, default=None, help='Custom CSV file name (e.g., train_clean.csv)')
...
def init_distributed_mode(config, args):
    ...
    print('| distributed init (rank {}, local_rank {})'.format(args.rank, args.local_rank))
...
def main():
    ...
    if USE_ROBUST_LOADER:
        print("Loading data with robust loader (handles corrupted images)")
        train_loader, val_loader = get_dataloaders_robust(config, args.train_csv)
    else:
        print("Loading data with standard loader")
...
echo "✅ Created Python 3.6 compatible train_robust.py"
echo "Now you can run: ./start_training_robust.sh"
